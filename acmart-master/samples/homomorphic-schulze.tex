...
\item For candidates $c$ and $d$, let $M(c, d)$ denote the maximum strength, or generalized margin of a path
	from $c$ to $d$ i.e.  \[ M(c, d) = \max \lbrace st (p) : \text{p is path from c to d in G} \rbrace\]
...
We first homomorphically compute the margin matrix from 
(valid) encrypted ballots, 
and then compute winners on the basis of the (decrypted) margin \cite{Pattinson:2017:SVE}
...
a ballot, $b$, we first generate a random permutation matrix, $\pi$,
and commit each column of $\pi$ using the Pedersen commitment scheme producing 
a list of commitments, $c_{\pi}$ (hiding phase).
Moreover, we keep the $\pi$ secret and publish the $c_{\pi}$ (make it public). 
We shuffle each row of $b$ by $\pi$ producing a row-shuffled 
...
and a honest decryption zero-knowledge proof, $decb_{zkprc\pi}$. We have
formally established that if the ballot $decb_{zkprc\pi}$ is 
\emph{valid}, then the ballot $b$ is valid as well, and vice versa. 
Only the valid ballots contribute to the final homomorphic margin matrix. 
Finally, we publish the whole data (except the secret permutation
$\pi$)  on the bulletin board (scrutiny sheet) to establish the verifiablity. Couple of important 
points to be noted:
\begin{itemize}
\item shuffle introduces re-encryption, so $\pi$ can not be guess from the ballot $b$ and 
 ...
 \item for the binding step in Pedersen commitment, rather than opening $\pi$ 
...
guarantee both provably correct and verifiable counting on the basis
of encrypted ballots, relative to an axiomatisation of the cryptographic primitives.
%cryptographic primitives. We then obtain, via program extraction, a
%provably correct implementation of vote counting, that we turn
%into executable code by providing implementations of the primitives
%based on a standard cryptographic library.  
Finally, in a nutshell, we achieve:
(i) \textit{correctness} by formally specifying the Schulze method  and prove its correctness properties
  ...
  final tally without decrypting any individual ballot, and (iii) \textit{verifiability} by tabulating the
   relevant data of election (we call it scrutiny-sheet/certificate).
